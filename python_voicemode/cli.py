...
# Import version info
try:
    from python_voicemode.version import __version__
except ImportError:
    __version__ = "unknown"
...
@click.pass_context
def python_voicemode_main_cli(ctx, debug, tools_enabled, tools_disabled):
    ...
    if ctx.invoked_subcommand is None:
        ...
        from .server import main as python_voicemode_main
        python_voicemode_main()


def python_voicemode() -> None:
...
@python_voicemode_main_cli.group()
...
def service_start(service_name):
    ...
    """
    from python_voicemode.tools.service import start_service
...
def service_stop(service_name):
    ...
    Services:
      ...
      voicemode  HTTP MCP server for remote access
    """
    from python_voicemode.tools.service import stop_service
...
def service_restart(service_name):
    ...
    Services:
      ...
      voicemode  HTTP MCP server for remote access
    """
    from python_voicemode.tools.service import restart_service
...
def service_status(service_name):
    ...
    """
    from python_voicemode.tools.service import status_service
...
def service_enable(service_name):
    ...
    Services:
      ...
      voicemode  HTTP MCP server for remote access
    """
    from python_voicemode.tools.service import enable_service
...
def service_disable(service_name):
    ...
    Services:
      ...
      voicemode  HTTP MCP server for remote access
    """
    from python_voicemode.tools.service import disable_service
...
def service_logs(service_name, lines):
    ...
    """
    from python_voicemode.tools.service import view_logs
...
def service_install(service_name, force):
    ...
    if service_name == 'whisper':
        ...
        from python_voicemode.tools.kokoro.install import kokoro_install
        ...
        from python_voicemode.tools.service import install_voicemode_start_script
...
@python_voicemode_main_cli.group(hidden=True)
@click.help_option('-h', '--help', help='Show this message and exit')
def kokoro():
...
def uninstall(remove_models, remove_all_data):
    ...
    from python_voicemode.tools.kokoro.uninstall import kokoro_uninstall
...
from python_voicemode.whisper_model_unified import whisper_model_unified
...
def whisper_model_list():
    ...
    """
    from python_voicemode.tools.whisper.models import (
...
def whisper_model_install(model, force, skip_core_ml):
    ...
    """
    import json
    import python_voicemode.tools.whisper.model_install as install_module
...
def config_list():
    ...
    from python_voicemode.tools.configuration_management import list_config_keys
...
def config_set(key, value):
    ...
    from python_voicemode.tools.configuration_management import update_config
...
def config_edit(editor):
    ...
    if not config_path.exists():
        ...
        from python_voicemode.config import load_voicemode_env
...
@python_voicemode_main_cli.command()
@click.help_option('-h', '--help')
...
def deps(component, yes, dry_run, verbose):
    ...
    """
    from python_voicemode.utils.dependencies.checker import (
...
def info():
    ...
    from python_voicemode.tools.diagnostics import python_voicemode_info
    result = asyncio.run(python_voicemode_info.fn())
    click.echo(result)
...
def devices():
    ...
    from python_voicemode.tools.devices import check_audio_devices
...
def registry():
    ...
    from python_voicemode.tools.voice_registry import voice_registry
...
from python_voicemode.cli_commands import exchanges as exchanges_cmd
from python_voicemode.cli_commands import transcribe as transcribe_cmd
from python_voicemode.cli_commands import status as status_cmd
from python_voicemode.cli_commands import claude as claude_cmd
from python_voicemode.cli_commands import soundfonts as soundfonts_cmd
from python_voicemode.cli_commands import voice as voice_cmd
...
python_voicemode_main_cli.add_command(exchanges_cmd.exchanges)
# Add unified status command
python_voicemode_main_cli.add_command(status_cmd.status)

# Add Claude Code integration commands
python_voicemode_main_cli.add_command(claude_cmd.claude)

# Add soundfonts toggle commands
python_voicemode_main_cli.add_command(soundfonts_cmd.soundfonts)
python_voicemode_main_cli.add_command(voice_cmd.voice)
...
python_voicemode_main_cli.add_command(transcribe_audio_cmd)

def _apply_converse_options(func):
    func = click.option('--continuous', '-c', is_flag=True, help='Continuous conversation mode')(func)
    func = click.option('--skip-tts/--no-skip-tts', default=None, help='Skip TTS and only show text')(func)
    func = click.option('--vad-aggressiveness', type=int, help='VAD aggressiveness (0-3)')(func)
    func = click.option('--speed', type=float, help='Speech rate (0.25 to 4.0)')(func)
    func = click.option('--disable-silence-detection', is_flag=True, help='Disable silence detection')(func)
    func = click.option('--audio-format', help='Audio format (pcm, mp3, wav, flac, aac, opus)')(func)
    func = click.option('--audio-feedback/--no-audio-feedback', default=None, help='Enable/disable audio feedback')(func)
    func = click.option('--tts-instructions', help='Tone/style instructions for gpt-4o-mini-tts')(func)
    func = click.option('--tts-model', help='TTS model (e.g., tts-1, tts-1-hd)')(func)
    func = click.option('--tts-provider', type=click.Choice(['openai', 'kokoro']), help='TTS provider')(func)
    func = click.option('--voice', help='TTS voice to use (e.g., nova, shimmer, af_sky)')(func)
    func = click.option('--min-duration', type=float, default=MIN_RECORDING_DURATION, help='Minimum listen duration before silence detection')(func)
    func = click.option('--duration', '-d', type=float, default=DEFAULT_LISTEN_DURATION, help='Listen duration in seconds')(func)
    func = click.option('--wait/--no-wait', default=True, help='Wait for response after speaking')(func)
    func = click.option('--message', '-m', default="Hello! How can I help you today?", help='Initial message to speak')(func)
    func = click.help_option('-h', '--help')(func)
    return func


def _run_converse_cli(message, wait, duration, min_duration, voice, tts_provider,
    ...
    """Run the shared converse/chat CLI flow."""
    from python_voicemode.utils.dependencies.checker import check_component_dependencies
    ...
    from python_voicemode.tools.converse import converse as converse_fn
...
def converse(message, wait, duration, min_duration, voice, tts_provider,
    ...
    """
    _run_converse_cli(
        message, wait, duration, min_duration, voice, tts_provider,
        tts_model, tts_instructions, audio_feedback, audio_format, disable_silence_detection,
        speed, vad_aggressiveness, skip_tts, continuous
    )


@python_voicemode_main_cli.command("chat")
@_apply_converse_options
def chat(message, wait, duration, min_duration, voice, tts_provider,
         tts_model, tts_instructions, audio_feedback, audio_format, disable_silence_detection,
         speed, vad_aggressiveness, skip_tts, continuous):
    """Short alias for `voicemode converse`."""
    _run_converse_cli(
        message, wait, duration, min_duration, voice, tts_provider,
        tts_model, tts_instructions, audio_feedback, audio_format, disable_silence_detection,
        speed, vad_aggressiveness, skip_tts, continuous
...
@python_voicemode_main_cli.command("c", hidden=True)
@_apply_converse_options
def chat_short_alias(message, wait, duration, min_duration, voice, tts_provider,
                     tts_model, tts_instructions, audio_feedback, audio_format, disable_silence_detection,
                     speed, vad_aggressiveness, skip_tts, continuous):
    """Hidden terse alias for `voicemode chat`."""
    _run_converse_cli(
        message, wait, duration, min_duration, voice, tts_provider,
        tts_model, tts_instructions, audio_feedback, audio_format, disable_silence_detection,
        speed, vad_aggressiveness, skip_tts, continuous
...
@python_voicemode_main_cli.command("read")
@click.help_option('-h', '--help')
@click.argument("path", type=click.Path(exists=True, dir_okay=False, path_type=Path))
def read_shortcut(path: Path):
    """Short alias for `voicemode voice read`."""
    response = voice_cmd.dispatch_voice_read(path)
    click.echo(json.dumps(response, indent=2, sort_keys=True))


@python_voicemode_main_cli.command("r", hidden=True)
@click.help_option('-h', '--help')
@click.argument("path", type=click.Path(exists=True, dir_okay=False, path_type=Path))
def read_short_alias(path: Path):
    """Hidden terse alias for `voicemode read`."""
    response = voice_cmd.dispatch_voice_read(path)
    click.echo(json.dumps(response, indent=2, sort_keys=True))


_MODE_CHOICES = ["walk", "walkie-talkie", "convo", "conversational"]


@python_voicemode_main_cli.command("mode")
@click.help_option('-h', '--help')
@click.argument("mode", type=click.Choice(_MODE_CHOICES))
def mode_shortcut(mode: str):
    """Switch the running runtime mode."""
    response = voice_cmd.dispatch_voice_control("MODE_SWITCH", mode)
    click.echo(json.dumps(response, indent=2, sort_keys=True))


@python_voicemode_main_cli.command("m", hidden=True)
@click.help_option('-h', '--help')
@click.argument("mode", type=click.Choice(_MODE_CHOICES))
def mode_short_alias(mode: str):
    """Hidden terse alias for `voicemode mode`."""
    response = voice_cmd.dispatch_voice_control("MODE_SWITCH", mode)
    click.echo(json.dumps(response, indent=2, sort_keys=True))


@python_voicemode_main_cli.command("stop")
@click.help_option('-h', '--help')
def stop_shortcut():
    """Stop current speech or read-aloud playback."""
    response = voice_cmd.dispatch_voice_control("TTS_STOP")
    click.echo(json.dumps(response, indent=2, sort_keys=True))


@python_voicemode_main_cli.command("x", hidden=True)
@click.help_option('-h', '--help')
def stop_short_alias():
    """Hidden terse alias for `voicemode stop`."""
    response = voice_cmd.dispatch_voice_control("TTS_STOP")
    click.echo(json.dumps(response, indent=2, sort_keys=True))
...
def play(source: str, chapters: str | None, volume: int):
    ...
    """
    from python_voicemode.dj import DJController
...
def login(no_browser: bool):
    ...
    """
    from python_voicemode.auth import login as auth_login, AuthError, format_expiry
...
def logout():
    ...
    Examples:
        voicemode connect auth logout
    """
    from python_voicemode.auth import load_credentials, clear_credentials
...
def auth_status():
    ...
    Examples:
        voicemode connect auth status
    """
    from python_voicemode.auth import get_valid_credentials, format_expiry, AuthError
...
async def _query_gateway_status(ws_url: str, access_token: str) -> dict:
    ...
    try:
        async with websockets.connect(
            ...
            from python_voicemode.connect import config as connect_config
            from python_voicemode.version import __version__
...
def status():
    ...
    """
    from python_voicemode.connect import config as connect_config
    from python_voicemode.connect.client import DeviceInfo
    ...
    gateway_result = None
    try:
        from python_voicemode.auth import get_valid_credentials
    ...
    if gateway_result and gateway_result["connected"]:
        ...
        import json
        from python_voicemode.connect.users import CONNECT_DIR
...
def user_add(name: str, display_name: str, subscribe_team: str | None):
    ...
    """
    import re
    from python_voicemode.connect.config import require_enabled, ConnectDisabledError, get_host
    from python_voicemode.connect.users import UserManager
