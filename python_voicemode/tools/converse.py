...
from python_voicemode.conversation_logger import get_conversation_logger
...
from python_voicemode.core import (
    get_openai_clients,
    text_to_speech,
...
from python_voicemode.audio_player import NonBlockingAudioPlayer
from python_voicemode.statistics_tracking import track_voice_interaction
from python_voicemode.utils import (
    get_event_logger,
    log_recording_start,
    log_recording_end,
    log_stt_start,
...
from python_voicemode.pronounce import get_manager as get_pronounce_manager, is_enabled as pronounce_enabled
...
async def startup_initialization():
    ...
    if python_voicemode.config._startup_initialized:
        return
    
    python_voicemode.config._startup_initialized = True
...
async def get_tts_config(provider: Optional[str] = None, voice: Optional[str] = None, model: Optional[str] = None, instructions: Optional[str] = None):
    ...
    from python_voicemode.provider_discovery import detect_provider_type
...
async def get_stt_config(provider: Optional[str] = None):
    ...
    from python_voicemode.provider_discovery import detect_provider_type
    from python_voicemode.config import STT_BASE_URLS
...
async def speech_to_text(
    ...
    import io
    from python_voicemode.conversation_logger import get_conversation_logger
    from python_voicemode.core import save_debug_file, get_debug_filename
    from python_voicemode.simple_failover import simple_stt_failover
    from python_voicemode.config import STT_BASE_URLS, STT_COMPRESS
...
def record_audio(duration: float) -> np.ndarray:
    ...
    try:
        ...
        from python_voicemode.utils.audio_diagnostics import get_audio_error_help
...
consult the MCP resources listed above.
    ...
    ffmpeg_available = getattr(python_voicemode.config, 'FFMPEG_AVAILABLE', True)  # Default to True if not set
            ...
            async with audio_operation_lock:
                ...
                if not tts_success:
                    ...
                    # Check if we have config info that might indicate why it failed
                        ...
                        from python_voicemode.config import OPENAI_API_KEY
