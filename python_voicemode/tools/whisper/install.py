...
from python_voicemode.config import SERVICE_AUTO_ENABLE, DEFAULT_WHISPER_MODEL, WHISPER_PORT
from python_voicemode.utils.services.whisper_helpers import download_whisper_model
from python_voicemode.utils.version_helpers import (
    get_git_tags, get_latest_stable_tag, get_current_version,
    checkout_version, is_version_installed
)
from python_voicemode.utils.migration_helpers import auto_migrate_if_needed
from python_voicemode.utils.gpu_detection import detect_gpu
...
async def update_whisper_service_files(
    ...
    if source_template.exists():
        ...
        try:
            template_resource = files("python_voicemode.templates.scripts").joinpath("start-whisper-server.sh")
...
"""
    ...
    if system == "Darwin":
        ...
        if source_template.exists():
            ...
            template_resource = files("python_voicemode.templates.launchd").joinpath("com.voicemode.whisper.plist")
        ...
        if source_template.exists():
            ...
            try:
                template_resource = files("python_voicemode.templates.systemd").joinpath("voicemode-whisper.service")
