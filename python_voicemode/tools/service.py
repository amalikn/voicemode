...
from python_voicemode.config import WHISPER_PORT, KOKORO_PORT, SERVICE_AUTO_ENABLE
from python_voicemode.utils.services.common import find_process_by_port, check_service_status

# Default port for VoiceMode serve command (HTTP MCP server)
VOICEMODE_SERVE_PORT = 8765
from python_voicemode.utils.services.whisper_helpers import find_whisper_server, find_whisper_model
from python_voicemode.utils.services.kokoro_helpers import find_kokoro_fastapi, has_gpu_support, is_kokoro_starting_up
...
def get_service_config_vars(service_name: str) -> Dict[str, Any]:
    ...
    if service_name == "whisper":
        ...
        from python_voicemode.config import KOKORO_MAX_REQUESTS
...
async def status_service(service_name: str) -> str:
   ...
   Type: WebSocket client""")
        ...
        from python_voicemode.connect.client import get_client
        client = get_client()
   ...
   Remote: Accessible"""
    ...
    try:
        ...
        if service_name == "whisper":
            ...
            try:
                from python_voicemode.utils.services.whisper_version import get_whisper_version_info, check_coreml_model_exists
            ...
            try:
                from python_voicemode.utils.services.version_info import get_kokoro_version
...
async def start_service(service_name: str) -> str:
    ...
    if service_name == "whisper":
        ...
        import sys
        cmd = [sys.executable, "-m", "python_voicemode", "serve", "--port", str(port)]

    elif service_name == "connect":
        # Start voicemode connect up directly
        import sys
        cmd = [sys.executable, "-m", "python_voicemode", "connect", "up"]
