...
class TestMinDurationIntegration:
    ...
    async def test_converse_validates_min_duration(self):
        ...
        with patch('python_voicemode.tools.converse.startup_initialization', new_callable=AsyncMock):
            ...
            python_voicemode.config.FFMPEG_AVAILABLE = True
            ...
            with patch('python_voicemode.tools.converse.logger') as mock_logger:
                with patch('python_voicemode.tools.converse.text_to_speech_with_failover', new_callable=AsyncMock) as mock_tts:
                    ...
                    with patch('python_voicemode.tools.converse.record_audio_with_silence_detection') as mock_record:
                        mock_record.return_value = (np.array([1, 2, 3]), True)  # Returns tuple (audio, speech_detected)
                        with patch('python_voicemode.tools.converse.speech_to_text', new_callable=AsyncMock) as mock_stt:
