...
from python_voicemode.provider_discovery import ProviderRegistry, EndpointInfo, detect_provider_type
from python_voicemode.providers import get_tts_client_and_voice, _select_model_for_endpoint
...
class TestVoiceFirstSelection:
    ...
    def mock_voice_preferences(self):
        ...
        with patch('python_voicemode.providers.get_voice_preferences', return_value=[]):
            yield
    ...
    async def test_voice_first_selects_kokoro_for_af_sky(self, mock_registry):
        ...
        with patch('python_voicemode.providers.provider_registry', mock_registry):
            with patch('python_voicemode.providers.get_voice_preferences', return_value=['af_sky', 'alloy']):
                with patch('python_voicemode.providers.TTS_MODELS', ['tts-1', 'tts-1-hd']):
                    with patch('python_voicemode.providers.TTS_BASE_URLS', [
    ...
    async def test_voice_first_selects_openai_for_nova(self, mock_registry):
        ...
        with patch('python_voicemode.providers.provider_registry', mock_registry):
            with patch('python_voicemode.providers.get_voice_preferences', return_value=['nova', 'af_sky']):
                with patch('python_voicemode.providers.TTS_MODELS', ['tts-1-hd', 'tts-1']):
                    with patch('python_voicemode.providers.TTS_BASE_URLS', [
                        'http://127.0.0.1:8880/v1',
                        'https://api.openai.com/v1'
                    ]):
    ...
    async def test_specific_voice_overrides_preferences(self, mock_registry):
        ...
        with patch('python_voicemode.providers.provider_registry', mock_registry):
            with patch('python_voicemode.providers.get_voice_preferences', return_value=['nova', 'alloy']):
                with patch('python_voicemode.providers.TTS_BASE_URLS', [
                    'http://127.0.0.1:8880/v1',
                    'https://api.openai.com/v1'
                ]):
    ...
    async def test_endpoint_with_error_still_tried(self, mock_registry):
        ...
        with patch('python_voicemode.providers.provider_registry', mock_registry):
            with patch('python_voicemode.providers.get_voice_preferences', return_value=['af_sky', 'nova']):
                with patch('python_voicemode.providers.TTS_BASE_URLS', [
                    'http://127.0.0.1:8880/v1',
                    'https://api.openai.com/v1'
                ]):
    ...
    async def test_model_selection_respects_provider_models(self, mock_registry):
        ...
        with patch('python_voicemode.providers.provider_registry', mock_registry):
            with patch('python_voicemode.providers.get_voice_preferences', return_value=['af_sky']):
                with patch('python_voicemode.providers.TTS_MODELS', ['gpt-4o-mini-tts', 'tts-1-hd', 'tts-1']):
                    with patch('python_voicemode.providers.TTS_BASE_URLS', [
                        'http://127.0.0.1:8880/v1',
                        'https://api.openai.com/v1'
                    ]):
