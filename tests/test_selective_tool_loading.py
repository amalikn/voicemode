...
from python_voicemode import tools

# Check which tool modules are loaded
loaded_modules = [m for m in sys.modules.keys() if 'python_voicemode.tools' in m]
print('LOADED:', sorted(loaded_modules))

# Verify statistics module is NOT loaded
assert 'python_voicemode.tools.statistics' not in sys.modules
assert 'python_voicemode.tools.converse' in sys.modules
print('SUCCESS')
"""],
        capture_output=True,
        text=True
    ...
    assert "python_voicemode.tools.converse" in result.stdout
    assert "python_voicemode.tools.statistics" not in result.stdout.replace("LOADED:", "")
...
assert 'python_voicemode.tools.converse' in sys.modules
assert 'python_voicemode.tools.statistics' in sys.modules
print('SUCCESS')
"""],
        capture_output=True,
        text=True
...
tool_count = len([m for m in loaded_modules if m.startswith('python_voicemode.tools.')])
print(f'Tool count: {tool_count}')
assert tool_count == 3, f"Expected 3 tools (converse, service, connect_status), got {tool_count}"
assert 'python_voicemode.tools.converse' in sys.modules
assert 'python_voicemode.tools.service' in sys.modules
assert 'python_voicemode.tools.connect_status' in sys.modules
print('SUCCESS')
"""],
        capture_output=True,
        text=True
