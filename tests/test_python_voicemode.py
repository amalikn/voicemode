...
@pytest.fixture
async def python_voicemode_server(mock_openai_clients, mock_audio_functions):
    ...
    script_path = Path(__file__).parent.parent / "src" / "python_voicemode" / "scripts" / "voice-mode"
    ...
    try:
        ...
        with patch.dict('sys.modules', {
            ...
            spec = importlib.util.spec_from_file_location("python_voicemode_script", tmp_path)
            python_voicemode_module = importlib.util.module_from_spec(spec)
            
            # Patch the get_openai_clients function
            with patch.object(python_voicemode_module, 'get_openai_clients', return_value=mock_openai_clients):
                spec.loader.exec_module(python_voicemode_module)
            
            # Return the configured MCP server
            return python_voicemode_module.mcp
    finally:
        os.unlink(tmp_path)
...
class TestVoiceMCPTools:
    ...
    @pytest.mark.asyncio
    async def test_speak_text(self, python_voicemode_server):
    ...
    @pytest.mark.asyncio
    async def test_listen_for_speech(self, python_voicemode_server):
    ...
    @pytest.mark.asyncio
    async def test_converse_local(self, python_voicemode_server):
    ...
    @pytest.mark.asyncio
    async def test_check_audio_devices(self, python_voicemode_server):
...
class TestErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_tts_api_error(self, python_voicemode_server, mock_openai_clients):
    ...
    @pytest.mark.asyncio
    async def test_stt_api_error(self, python_voicemode_server, mock_openai_clients):
    ...
    @pytest.mark.asyncio
    async def test_recording_error(self, python_voicemode_server):
    ...
    def test_environment_variables(self):
        ...
        try:
            ...
            with patch.dict('sys.modules', {
                ...
                assert python_voicemode_module.STT_BASE_URL == 'http://127.0.0.1:2022/v1'
                assert python_voicemode_module.TTS_BASE_URL == 'http://127.0.0.1:8880/v1'
                assert python_voicemode_module.TTS_VOICE == 'custom_voice'
                assert python_voicemode_module.TTS_MODEL == 'custom-tts'
                assert python_voicemode_module.STT_MODEL == 'custom-stt'
        finally:
            os.unlink(tmp_path)
    ...
    def test_debug_mode(self):
        ...
        try:
            ...
            with patch.dict('sys.modules', {
                ...
                assert python_voicemode_module.DEBUG == True
                assert python_voicemode_module.DEBUG_DIR.exists()
        finally:
            os.unlink(tmp_path)
...
class TestAudioProcessing:
    ...
    @pytest.mark.skip(reason="Complex mocking of script-based MCP server not yet implemented")
    @pytest.mark.asyncio
    async def test_audio_file_formats(self, python_voicemode_server, mock_openai_clients):
...
class TestLiveKitIntegration:
    ...
    @pytest.mark.asyncio
    async def test_livekit_availability_check(self, python_voicemode_server):
        """Test LiveKit availability checking"""
        # Mock check_livekit_available to return False
        async def mock_check_livekit():
            return False
            
        with patch.object(python_voicemode_server.app, 'check_livekit_available', mock_check_livekit):
            async with Client(python_voicemode_server) as client:
...
class TestDebugFeatures:
    ...
    @pytest.mark.asyncio
    async def test_debug_file_saving(self, python_voicemode_server, tmp_path):
        ...
        for name, module in sys.modules.items():
            if 'python_voicemode_script' in name:
                voice_module = module
                break
...
@pytest.mark.skip(reason="Complex mocking of script-based MCP server not yet implemented")
@pytest.mark.asyncio
async def test_full_conversation_flow(python_voicemode_server):
